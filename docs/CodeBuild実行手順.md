...
# イメージ名を変更したい場合
.\setup-codebuild.ps1 -AccountId "YOUR_AWS_ACCOUNT_ID" -GitHubUsername "your-github-username" -RepositoryName "my-windows-app"
```
...
2. **codebuild/project.json** - REPOSITORY_URI環境変数
...
```
**解決策**: 
- `execution/build.bat` でローカルビルドを実行
- または事前にコンパイル済みの .exe ファイルをリポジトリに含める
...
CodeBuildプロジェクトでGitHubソースを設定し、Webhookを有効化することで、pushイベント時に自動ビルドが実行されます。
